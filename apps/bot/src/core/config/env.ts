...
// discord.js reads process.env.SHARD_COUNT directly. Empty/non-numeric values
// crash the single-process Client. Strip them so it falls back to default of 1.
for (const key of ["SHARD_COUNT", "TOTAL_SHARDS"] as const) {
  const v = process.env[key];
  if (v === undefined) continue;
  const n = Number(v);
  if (!Number.isFinite(n) || n < 1) delete process.env[key];
}
