...
import { runTicketAutoClose } from "./jobs/tickets-auto-close";
...
const RECURRING: Array<{ name: JobName; every: number }> = [
  ...
  { name: "reminders", every: 30_000 },
  { name: "tickets-auto-close", every: 5 * 60_000 },
];
  ...
  worker = new Worker(
    ...
    async (job: Job) => {
      switch (job.name as JobName) {
        ...
        case "reminders":
          return runReminders(client);
        case "tickets-auto-close":
          return runTicketAutoClose(client);
        default:
