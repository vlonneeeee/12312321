  ...
  async execute(interaction) {
    ...
    if (!SUPPORTED_LOCALES.includes(langRaw as Locale)) {
      await interaction.reply({
        embeds: [infoEmbed("Language", tWithLocale("en", "language.unsupported"))],
        ephemeral: true,
      });
      return;
    }
    ...
    await interaction.reply({
      embeds: [successEmbed("Language", tWithLocale(lang, "language.changed"))],
      ephemeral: true,
    });
  },
};
...
const showLanguage: SlashCommand = {
  ...
  async execute(interaction) {
    ...
    await interaction.reply({
      embeds: [infoEmbed("Language", tWithLocale(lang, "language.current", { lang }))],
      ephemeral: true,
    });
  },
};
