...
async function reply(interaction: ChatInputCommandInteraction, fn: () => Promise<void>) {
  try {
    ...
    const title = await t(gid, "moderation.title");
    ...
    else await interaction.reply({ embeds: [errorEmbed(title, msg)], ephemeral: true });
  }
}
...
async function fetchTarget(
  ...
  if (!user) throw new UserFacingError(await t(interaction.guildId, "moderation.no_target"));
  const member = await interaction.guild!.members.fetch(user.id).catch(() => null);
  if (!member)
    throw new UserFacingError(await t(interaction.guildId, "moderation.target_not_in_guild"));
  return member;
}
...
const warn: SlashCommand = {
  ...
  async execute(interaction) {
    await reply(interaction, async () => {
      ...
      const title = await t(interaction.guildId, "moderation.warned_title", {
        target: subject.user.tag,
      });
      const body = await t(interaction.guildId, "moderation.case_id", { id: record.id });
      await interaction.reply({ embeds: [successEmbed(title, body)] });
    });
  },
};
...
const mute: SlashCommand = {
  ...
  async execute(interaction) {
    await reply(interaction, async () => {
      ...
      const title = await t(interaction.guildId, "moderation.muted_title", {
        target: subject.user.tag,
      });
      await interaction.reply({ embeds: [successEmbed(title, `${seconds}s`)] });
    });
  },
};
...
const kick: SlashCommand = {
  ...
  async execute(interaction) {
    await reply(interaction, async () => {
      ...
      const title = await t(interaction.guildId, "moderation.kicked_title", {
        target: subject.user.tag,
      });
      await interaction.reply({ embeds: [successEmbed(title, reason ?? "—")] });
    });
  },
};
...
const ban: SlashCommand = {
  ...
  async execute(interaction) {
    await reply(interaction, async () => {
      ...
      const title = await t(interaction.guildId, "moderation.banned_title", {
        target: subject.user.tag,
      });
      await interaction.reply({ embeds: [successEmbed(title, reason ?? "—")] });
    });
...
const softban: SlashCommand = {
  ...
  async execute(interaction) {
    await reply(interaction, async () => {
      ...
      const title = await t(interaction.guildId, "moderation.softbanned_title", {
        target: subject.user.tag,
      });
      await interaction.reply({ embeds: [successEmbed(title, reason ?? "—")] });
    });
...
const purge: SlashCommand = {
  ...
  async execute(interaction) {
    await reply(interaction, async () => {
      ...
      const title = await t(interaction.guildId, "moderation.purged_title", { count: deleted });
      await interaction.reply({ embeds: [successEmbed(title, "")], ephemeral: true });
    });
  },
};
...
const cases: SlashCommand = {
  ...
  async execute(interaction) {
    await reply(interaction, async () => {
      ...
      const noCases = await t(interaction.guildId, "moderation.no_cases");
      const lines = records.length
        ? records
            ...
            .join("\n")
        : noCases;
      const title = await t(interaction.guildId, "moderation.cases_title", {
        target: user.tag,
      });
      await interaction.reply({ embeds: [infoEmbed(title, lines)] });
    });
  },
};
